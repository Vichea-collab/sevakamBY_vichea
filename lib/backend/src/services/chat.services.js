import admin from "firebase-admin";
import { db, storage } from "../config/firebase.js";
import { env } from "../config/env.js";
import { paginateArray } from "../utils/pagination.util.js";
import PushService from "./push.services.js";

const CHAT_COLLECTION = "chats";
const MAX_MESSAGES = 120;
const MAX_THREADS = 120;
const MAX_IMAGE_BYTES = Number.isFinite(env.CHAT_IMAGE_MAX_BYTES)
  ? Math.max(256 * 1024, Math.floor(env.CHAT_IMAGE_MAX_BYTES))
  : 10 * 1024 * 1024;
const INLINE_MAX_IMAGE_BYTES = Number.isFinite(env.CHAT_IMAGE_INLINE_MAX_BYTES)
  ? Math.max(64 * 1024, Math.floor(env.CHAT_IMAGE_INLINE_MAX_BYTES))
  : 700 * 1024;

function readWindow(pagination, maxLimit) {
  const page = Math.max(1, Number.parseInt((pagination?.page ?? 1).toString(), 10) || 1);
  const limit = Math.max(1, Number.parseInt((pagination?.limit ?? 10).toString(), 10) || 10);
  return Math.min(maxLimit, page * limit + 1);
}

function normalizePagination(pagination, maxLimit) {
  return {
    page: Math.max(1, Number.parseInt((pagination?.page ?? 1).toString(), 10) || 1),
    limit: Math.min(
      maxLimit,
      Math.max(1, Number.parseInt((pagination?.limit ?? 10).toString(), 10) || 10),
    ),
    cursor: (pagination?.cursor ?? "").toString().trim(),
  };
}

async function applyCollectionCursor(query, collectionRef, cursor) {
  const cursorId = (cursor || "").toString().trim();
  if (!cursorId) return query;
  const snap = await collectionRef.doc(cursorId).get();
  if (!snap.exists) return query;
  return query.startAfter(snap);
}

function normalizeRole(value) {
  const role = (value || "").toString().trim().toLowerCase();
  if (role === "providers") return "provider";
  if (role === "finders") return "finder";
  return role;
}

function safeString(value, fallback = "") {
  const text = (value ?? "").toString().trim();
  return text.length === 0 ? fallback : text;
}

function directChatId(uidA, roleA, uidB, roleB) {
  const endpoints = [
    `${uidA.toString().trim()}:${normalizeRole(roleA) || "finder"}`,
    `${uidB.toString().trim()}:${normalizeRole(roleB) || "finder"}`,
  ].sort();
  return `direct_${endpoints[0].replace(":", "_")}_${endpoints[1].replace(":", "_")}`;
}

function toIsoDate(value) {
  if (!value) return new Date().toISOString();
  if (typeof value?.toDate === "function") {
    return value.toDate().toISOString();
  }
  if (value instanceof Date) {
    return value.toISOString();
  }
  if (typeof value === "string") {
    const parsed = new Date(value);
    if (!Number.isNaN(parsed.getTime())) return parsed.toISOString();
  }
  if (typeof value === "object" && value._seconds) {
    const parsed = new Date(Number(value._seconds) * 1000);
    if (!Number.isNaN(parsed.getTime())) return parsed.toISOString();
  }
  return new Date().toISOString();
}

function toInt(value, fallback = 0) {
  if (typeof value === "number" && Number.isFinite(value)) {
    return Math.round(value);
  }
  if (typeof value === "string") {
    const parsed = Number.parseInt(value, 10);
    if (Number.isFinite(parsed)) return parsed;
  }
  return fallback;
}

function normalizeUnreadCounts(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  const next = {};
  Object.entries(value).forEach(([uid, unread]) => {
    const key = safeString(uid);
    if (!key) return;
    next[key] = Math.max(0, toInt(unread, 0));
  });
  return next;
}

function withUnreadCount(unreadCounts, uid, count) {
  const key = safeString(uid);
  if (!key) return normalizeUnreadCounts(unreadCounts);
  const next = normalizeUnreadCounts(unreadCounts);
  next[key] = Math.max(0, toInt(count, 0));
  return next;
}

async function markThreadUnreadZero(chatRef, uid) {
  await db.runTransaction(async (transaction) => {
    const snap = await transaction.get(chatRef);
    if (!snap.exists) return;
    const row = snap.data() || {};
    transaction.set(
      chatRef,
      {
        unreadCounts: withUnreadCount(row.unreadCounts, uid, 0),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true },
    );
  });
}

async function fetchUserActiveTimes(uids = []) {
  const uniqueUids = Array.from(new Set(uids.map(u => safeString(u)).filter(Boolean)));
  if (uniqueUids.length === 0) return {};
  
  const results = {};
  // Firestore IN query supports up to 30 items
  const chunks = [];
  for (let i = 0; i < uniqueUids.length; i += 30) {
    chunks.push(uniqueUids.slice(i, i + 30));
  }
  
  for (const chunk of chunks) {
    const snap = await db.collection("users").where(admin.firestore.FieldPath.documentId(), "in", chunk).get();
    snap.docs.forEach(doc => {
      const data = doc.data();
      results[doc.id] = toIsoDate(data.lastActiveAt);
    });
  }
  return results;
}

function mapThreadDoc(doc, currentUid, userActiveTimes = {}) {
  const row = doc.data() || {};
  const participants = Array.isArray(row.participants)
    ? row.participants.map((value) => (value || "").toString().trim()).filter(Boolean)
    : [];
  if (!participants.includes(currentUid)) return null;

  const peerUid = participants.find((uid) => uid !== currentUid) || "";
  const participantMeta =
    row.participantMeta && typeof row.participantMeta === "object"
      ? row.participantMeta
      : {};
  const peerMeta =
    peerUid && participantMeta[peerUid] && typeof participantMeta[peerUid] === "object"
      ? participantMeta[peerUid]
      : {};
  const unreadCounts =
    row.unreadCounts && typeof row.unreadCounts === "object" ? row.unreadCounts : {};

  const lastMessageText = safeString(row.lastMessageText, "Start conversation");
  const lastSenderUid = safeString(row.lastSenderUid);
  const subtitle =
    lastSenderUid && lastSenderUid === currentUid && lastMessageText !== "Start conversation"
      ? `You: ${lastMessageText}`
      : lastMessageText;

  return {
    id: doc.id,
    title: safeString(peerMeta.name, "Chat"),
    subtitle,
    avatarPath: "assets/images/profile.jpg",
    updatedAt: toIsoDate(row.lastMessageAt || row.updatedAt || row.createdAt),
    lastActiveAt: userActiveTimes[peerUid] || toIsoDate(peerMeta.lastActiveAt),
    unreadCount: toInt(unreadCounts[currentUid], 0),
    peerUid,
  };
}

function safeStringList(value) {
  if (!Array.isArray(value)) return [];
  return value
    .map((item) => safeString(item))
    .filter((item) => item.length > 0);
}

function mapMessageDoc(doc) {
  const row = doc.data() || {};
  const seenBy = safeStringList(row.seenBy);
  const deliveredTo = safeStringList(row.deliveredTo);
  return {
    id: doc.id,
    text: safeString(row.text),
    type: safeString(row.type, "text"),
    imageUrl: safeString(row.imageUrl),
    senderUid: safeString(row.senderUid),
    senderName: safeString(row.senderName, "User"),
    sentAt: toIsoDate(row.sentAt || row.createdAt),
    seenBy,
    deliveredTo,
  };
}

async function markMessagesDeliveredForUid(uid, docs = []) {
  if (!uid || !Array.isArray(docs) || docs.length === 0) return 0;
  const batch = db.batch();
  let changed = 0;
  docs.forEach((doc) => {
    const row = doc.data() || {};
    const senderUid = safeString(row.senderUid);
    if (!senderUid || senderUid === uid) return;
    const deliveredTo = safeStringList(row.deliveredTo);
    if (deliveredTo.includes(uid)) return;
    batch.set(
      doc.ref,
      {
        deliveredTo: admin.firestore.FieldValue.arrayUnion(uid),
      },
      { merge: true },
    );
    changed += 1;
  });
  if (changed > 0) {
    await batch.commit();
  }
  return changed;
}

async function markMessagesSeenForUid(uid, docs = []) {
  if (!uid || !Array.isArray(docs) || docs.length === 0) return 0;
  const batch = db.batch();
  let changed = 0;
  docs.forEach((doc) => {
    const row = doc.data() || {};
    const senderUid = safeString(row.senderUid);
    if (!senderUid || senderUid === uid) return;
    const seenBy = safeStringList(row.seenBy);
    const deliveredTo = safeStringList(row.deliveredTo);
    const payload = {};
    if (!seenBy.includes(uid)) {
      payload.seenBy = admin.firestore.FieldValue.arrayUnion(uid);
    }
    if (!deliveredTo.includes(uid)) {
      payload.deliveredTo = admin.firestore.FieldValue.arrayUnion(uid);
    }
    if (Object.keys(payload).length === 0) return;
    batch.set(doc.ref, payload, { merge: true });
    changed += 1;
  });
  if (changed > 0) {
    await batch.commit();
  }
  return changed;
}

function extensionFromMime(mimeType) {
  const type = (mimeType || "").toString().trim().toLowerCase();
  switch (type) {
    case "image/png":
      return ".png";
    case "image/webp":
      return ".webp";
    case "image/gif":
      return ".gif";
    case "image/heic":
      return ".heic";
    case "image/jpeg":
    case "image/jpg":
    default:
      return ".jpg";
  }
}

function parseImageDataUrl(value) {
  const raw = (value || "").toString().trim();
  if (!raw) return null;
  const match = raw.match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,([A-Za-z0-9+/=]+)$/);
  if (!match) return null;
  const mimeType = match[1].toLowerCase();
  const base64 = match[2];
  const bytes = Buffer.from(base64, "base64");
  if (!bytes || bytes.length === 0) return null;
  return { mimeType, bytes, raw };
}

async function uploadChatImage({ threadId, messageId, imageDataUrl, fileName }) {
  const parsed = parseImageDataUrl(imageDataUrl);
  if (!parsed) {
    const error = new Error("invalid imageDataUrl");
    error.status = 400;
    throw error;
  }
  if (parsed.bytes.length > MAX_IMAGE_BYTES) {
    const error = new Error(
      `image is too large (max ${Math.floor(MAX_IMAGE_BYTES / 1024 / 1024)}MB)`,
    );
    error.status = 400;
    throw error;
  }

  try {
    const bucket = storage.bucket();
    const extension = extensionFromMime(parsed.mimeType);
    const sanitizedFileName = safeString(fileName || "", `image${extension}`)
      .replace(/[^a-zA-Z0-9._-]/g, "_")
      .replace(/_+/g, "_");
    const objectPath = `chat_uploads/${threadId}/${Date.now()}_${messageId}_${sanitizedFileName}`;
    const file = bucket.file(objectPath);

    await file.save(parsed.bytes, {
      resumable: false,
      metadata: {
        contentType: parsed.mimeType,
        cacheControl: "public,max-age=31536000",
      },
    });

    const [signedUrl] = await file.getSignedUrl({
      action: "read",
      expires: "03-01-2500",
    });

    return {
      imageUrl: signedUrl,
      imagePath: objectPath,
      mimeType: parsed.mimeType,
    };
  } catch (uploadError) {
    const allowInline = env.CHAT_IMAGE_INLINE_FALLBACK === true;
    if (allowInline && parsed.bytes.length <= INLINE_MAX_IMAGE_BYTES) {
      return {
        imageUrl: parsed.raw,
        imagePath: "__inline__",
        mimeType: parsed.mimeType,
      };
    }

    const reason = safeString(uploadError?.message, "Storage upload failed");
    const error = new Error(`storage upload failed: ${reason}`);
    error.status = 500;
    throw error;
  }
}

function resolveRemoteImageMeta({ imageUrl, mimeType }) {
  const resolvedUrl = safeString(imageUrl);
  if (!resolvedUrl) return null;
  return {
    imageUrl: resolvedUrl,
    imagePath: "",
    mimeType: safeString(mimeType),
  };
}

async function ensureChatParticipant(uid, threadId) {
  const chatRef = db.collection(CHAT_COLLECTION).doc(threadId);
  const chatSnap = await chatRef.get();
  if (!chatSnap.exists) {
    const error = new Error("chat not found");
    error.status = 404;
    throw error;
  }
  const row = chatSnap.data() || {};
  const participants = Array.isArray(row.participants)
    ? row.participants.map((value) => (value || "").toString().trim()).filter(Boolean)
    : [];
  if (!participants.includes(uid)) {
    const error = new Error("forbidden");
    error.status = 403;
    throw error;
  }
  return { chatRef, chatSnap, participants };
}

async function mapAndSortThreads(docs, currentUid) {
  const peerUids = docs.map(doc => {
    const row = doc.data() || {};
    const participants = Array.isArray(row.participants) ? row.participants : [];
    return participants.find(uid => uid !== currentUid);
  }).filter(Boolean);
  
  const userActiveTimes = await fetchUserActiveTimes(peerUids);

  return docs
    .map((doc) => mapThreadDoc(doc, currentUid, userActiveTimes))
    .filter(Boolean)
    .sort((a, b) => {
      const aTime = new Date(a.updatedAt).getTime();
      const bTime = new Date(b.updatedAt).getTime();
      return bTime - aTime;
    });
}

function matchesRoleScope(row, uid, role) {
  const normalizedRole = normalizeRole(role);
  if (!["finder", "provider"].includes(normalizedRole)) return true;
  const participantMeta =
    row?.participantMeta && typeof row.participantMeta === "object"
      ? row.participantMeta
      : {};
  const currentMeta =
    uid && participantMeta[uid] && typeof participantMeta[uid] === "object"
      ? participantMeta[uid]
      : {};
  return normalizeRole(currentMeta.role) === normalizedRole;
}

function isIndexRequirementError(error) {
  const code = safeString(error?.code).toLowerCase();
  const message = safeString(error?.message).toLowerCase();
  return (
    code === "failed-precondition" ||
    code === "9" ||
    message.includes("requires an index") ||
    message.includes("failed precondition")
  );
}

class ChatService {
  static async heartbeat(uid) {
    if (!uid) return;
    const userRef = db.collection("users").doc(uid);
    await userRef.set(
      {
        lastActiveAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true },
    );
    return { success: true };
  }

  static async getUnreadCount(uid, role = "") {
    const snap = await db
      .collection(CHAT_COLLECTION)
      .where("participants", "array-contains", uid)
      .limit(MAX_THREADS)
      .get();

    const summary = snap.docs.reduce(
      (accumulator, doc) => {
        const row = doc.data() || {};
        if (!matchesRoleScope(row, uid, role)) {
          return accumulator;
        }
        const unreadCounts =
          row.unreadCounts && typeof row.unreadCounts === "object"
            ? row.unreadCounts
            : {};
        const unreadMessages = Math.max(0, toInt(unreadCounts[uid], 0));
        if (unreadMessages > 0) {
          accumulator.unreadThreadCount += 1;
          accumulator.unreadMessageCount += unreadMessages;
        }
        return accumulator;
      },
      { unreadThreadCount: 0, unreadMessageCount: 0 },
    );

    return {
      data: {
        unreadCount: summary.unreadThreadCount,
        unreadThreadCount: summary.unreadThreadCount,
        unreadMessageCount: summary.unreadMessageCount,
      },
    };
  }

  static async listThreads(uid, pagination, role = "") {
    const normalized = normalizePagination(pagination, MAX_THREADS);
    const canUseCursor = normalized.page === 1 || Boolean(normalized.cursor);
    try {
      if (canUseCursor) {
        const chatsRef = db.collection(CHAT_COLLECTION);
        let queryRef = chatsRef
          .where("participants", "array-contains", uid)
          .orderBy("lastMessageAt", "desc");
        queryRef = await applyCollectionCursor(queryRef, chatsRef, normalized.cursor);
        const fetchLimit = normalized.limit + 1;
        const snap = await queryRef.limit(fetchLimit).get();
        let docs = snap.docs;
        const hasNextPage = docs.length > normalized.limit;
        if (docs.length > normalized.limit) {
          docs = docs.slice(0, normalized.limit);
        }
        const threads = await mapAndSortThreads(
          docs.filter((doc) => matchesRoleScope(doc.data() || {}, uid, role)),
          uid,
        );
        return {
          data: threads,
          pagination: {
            page: normalized.page,
            limit: normalized.limit,
            totalItems: threads.length + (hasNextPage ? 1 : 0),
            totalPages: hasNextPage ? normalized.page + 1 : normalized.page,
            hasPrevPage: Boolean(normalized.cursor) || normalized.page > 1,
            hasNextPage,
            nextCursor: hasNextPage && docs.length > 0 ? docs[docs.length - 1].id : "",
          },
        };
      }
      const snap = await db
        .collection(CHAT_COLLECTION)
        .where("participants", "array-contains", uid)
        .orderBy("lastMessageAt", "desc")
        .limit(readWindow(normalized, MAX_THREADS))
        .get();
      const threads = await mapAndSortThreads(
        snap.docs.filter((doc) => matchesRoleScope(doc.data() || {}, uid, role)),
        uid,
      );
      const paged = paginateArray(threads, normalized);
      return { data: paged.items, pagination: paged.pagination };
    } catch (error) {
      if (!isIndexRequirementError(error)) {
        throw error;
      }
      const snap = await db
        .collection(CHAT_COLLECTION)
        .where("participants", "array-contains", uid)
        .limit(MAX_THREADS)
        .get();
      const threads = await mapAndSortThreads(
        snap.docs.filter((doc) => matchesRoleScope(doc.data() || {}, uid, role)),
        uid,
      );
      const paged = paginateArray(threads, normalized);
      return { data: paged.items, pagination: paged.pagination };
    }
  }

  static async openDirectThread(uid, user, payload) {
    const peerUid = safeString(payload.peerUid);
    if (!peerUid) {
      const error = new Error("peerUid is required");
      error.status = 400;
      throw error;
    }
    if (peerUid === uid) {
      const error = new Error("Cannot chat with yourself");
      error.status = 400;
      throw error;
    }

    const selfRole = normalizeRole(payload.selfRole || user?.role) === "provider"
      ? "provider"
      : "finder";
    const peerRole = payload.peerIsProvider === true ? "provider" : "finder";
    const chatId = directChatId(uid, selfRole, peerUid, peerRole);
    const chatRef = db.collection(CHAT_COLLECTION).doc(chatId);
    const chatSnap = await chatRef.get();
    const exists = chatSnap.exists;

    const selfName = safeString(payload.selfName || user?.name, "User");
    const peerName = safeString(payload.peerName, "User");
    const now = admin.firestore.FieldValue.serverTimestamp();
    const peerIsProvider = peerRole === "provider";

    if (!exists) {
      await chatRef.set({
        id: chatId,
        kind: "direct",
        participants: [uid, peerUid],
        participantMeta: {
          [uid]: {
            name: selfName,
            role: selfRole === "provider" ? "provider" : "finder",
            avatarPath: "assets/images/profile.jpg",
          },
          [peerUid]: {
            name: peerName,
            role: peerIsProvider ? "provider" : "finder",
            avatarPath: "assets/images/profile.jpg",
          },
        },
        unreadCounts: {
          [uid]: 0,
          [peerUid]: 0,
        },
        lastMessageText: "",
        lastSenderUid: "",
        createdAt: now,
        updatedAt: now,
        lastMessageAt: now,
      });

      const starterText = safeString(
        payload.starterText,
        peerIsProvider
          ? `Hi ${peerName}, I want to discuss your service.`
          : `Hi ${peerName}, I can help with your request.`,
      );
      const msgRef = chatRef.collection("messages").doc();
      await msgRef.set({
        id: msgRef.id,
        text: starterText,
        senderUid: uid,
        senderName: selfName,
        sentAt: now,
        seenBy: [uid],
        deliveredTo: [uid],
      });
      await chatRef.set(
        {
          lastMessageText: starterText,
          lastSenderUid: uid,
          lastMessageAt: now,
          updatedAt: now,
          unreadCounts: {
            [uid]: 0,
            [peerUid]: 1,
          },
        },
        { merge: true },
      );
    } else {
      const unreadCounts = withUnreadCount((chatSnap.data() || {}).unreadCounts, uid, 0);
      await chatRef.set(
        {
          id: chatId,
          kind: "direct",
          participants: [uid, peerUid],
          [`participantMeta.${uid}`]: {
            name: selfName,
            role: selfRole === "provider" ? "provider" : "finder",
            avatarPath: "assets/images/profile.jpg",
          },
          [`participantMeta.${peerUid}`]: {
            name: peerName,
            role: peerIsProvider ? "provider" : "finder",
            avatarPath: "assets/images/profile.jpg",
          },
          unreadCounts,
          updatedAt: now,
        },
        { merge: true },
      );
    }

    const finalSnap = await chatRef.get();
    const userActiveTimes = await fetchUserActiveTimes([peerUid]);
    const thread = mapThreadDoc(finalSnap, uid, userActiveTimes);
    return { data: thread };
  }

  static async listMessages(uid, threadId, pagination) {
    const { chatRef } = await ensureChatParticipant(uid, threadId);
    const normalized = normalizePagination(pagination, MAX_MESSAGES);
    const canUseCursor = normalized.page === 1 || Boolean(normalized.cursor);
    if (canUseCursor) {
      const messagesRef = chatRef.collection("messages");
      let queryRef = messagesRef.orderBy("sentAt", "desc");
      queryRef = await applyCollectionCursor(queryRef, messagesRef, normalized.cursor);
      const fetchLimit = normalized.cursor ? normalized.limit + 1 : normalized.limit;
      const snap = await queryRef.limit(fetchLimit).get();
      let docs = snap.docs;
      const hasNextPage = normalized.cursor
        ? docs.length > normalized.limit
        : docs.length >= normalized.limit;
      if (normalized.cursor && docs.length > normalized.limit) {
        docs = docs.slice(0, normalized.limit);
      }
      await markMessagesSeenForUid(uid, docs);
      await markThreadUnreadZero(chatRef, uid);
      const items = docs.map(mapMessageDoc);
      return {
        data: items,
        pagination: {
          page: normalized.page,
          limit: normalized.limit,
          totalItems: items.length + (hasNextPage ? 1 : 0),
          totalPages: hasNextPage ? normalized.page + 1 : normalized.page,
          hasPrevPage: Boolean(normalized.cursor) || normalized.page > 1,
          hasNextPage,
          nextCursor: hasNextPage && docs.length > 0 ? docs[docs.length - 1].id : "",
        },
      };
    }
    const snap = await chatRef
      .collection("messages")
      .orderBy("sentAt", "desc")
      .limit(readWindow(pagination, MAX_MESSAGES))
      .get();
    await markMessagesSeenForUid(uid, snap.docs);
    await markThreadUnreadZero(chatRef, uid);
    const items = snap.docs.map(mapMessageDoc);
    const paged = paginateArray(items, pagination);
    return { data: paged.items, pagination: paged.pagination };
  }

  static async sendMessage(uid, user, threadId, payload) {
    const text = safeString(payload.text);
    const requestedType = safeString(payload.type, "");
    const imageUrl = safeString(payload.imageUrl);
    const imageDataUrl = safeString(payload.imageDataUrl);
    const isImage =
      requestedType === "image" || imageUrl.length > 0 || imageDataUrl.length > 0;

    if (!isImage && !text) {
      const error = new Error("text is required");
      error.status = 400;
      throw error;
    }

    const { chatRef, participants } = await ensureChatParticipant(uid, threadId);
    const msgRef = chatRef.collection("messages").doc();
    const now = admin.firestore.FieldValue.serverTimestamp();
    const senderName = safeString(payload.senderName || user?.name, "User");
    const recentBeforeReply = await chatRef
      .collection("messages")
      .orderBy("sentAt", "desc")
      .limit(120)
      .get();
    await markMessagesSeenForUid(uid, recentBeforeReply.docs);
    let imageMeta = null;
    if (isImage) {
      imageMeta =
        resolveRemoteImageMeta({
          imageUrl,
          mimeType: payload.mimeType,
        }) ||
        (await uploadChatImage({
          threadId,
          messageId: msgRef.id,
          imageDataUrl,
          fileName: payload.fileName,
        }));
    }

    const messageType = isImage ? "image" : "text";
    const previewText = isImage
      ? (text || "Photo")
      : text;

    await msgRef.set({
      id: msgRef.id,
      text,
      type: messageType,
      imageUrl: imageMeta?.imageUrl || "",
      imagePath: imageMeta?.imagePath || "",
      imageMimeType: imageMeta?.mimeType || "",
      senderUid: uid,
      senderName,
      sentAt: now,
      seenBy: [uid],
      deliveredTo: [uid],
    });

    await db.runTransaction(async (transaction) => {
      const latestChatSnap = await transaction.get(chatRef);
      if (!latestChatSnap.exists) {
        const error = new Error("chat not found");
        error.status = 404;
        throw error;
      }
      const latestRow = latestChatSnap.data() || {};
      const unreadCounts = withUnreadCount(latestRow.unreadCounts, uid, 0);
      participants.forEach((participantUid) => {
        const peerUid = safeString(participantUid);
        if (!peerUid || peerUid === uid) return;
        unreadCounts[peerUid] = Math.max(0, toInt(unreadCounts[peerUid], 0) + 1);
      });
      transaction.set(
        chatRef,
        {
          lastMessageText: previewText,
          lastSenderUid: uid,
          lastMessageAt: now,
          updatedAt: now,
          unreadCounts,
        },
        { merge: true },
      );
    });

    const recipientUids = participants
      .map((participantUid) => safeString(participantUid))
      .filter((participantUid) => participantUid && participantUid !== uid);
    await Promise.all(
      recipientUids.map((recipientUid) =>
        PushService.sendToUser({
          uid: recipientUid,
          title: `New message from ${senderName}`,
          body: previewText || "You have a new message.",
          data: {
            target: "chat",
            threadId,
            chatId: threadId,
          },
        }).catch(() => null),
      ),
    );

    const finalMessage = await msgRef.get();
    return { data: mapMessageDoc(finalMessage) };
  }

  static async markAsRead(uid, threadId) {
    const { chatRef } = await ensureChatParticipant(uid, threadId);

    const messagesSnap = await chatRef
      .collection("messages")
      .orderBy("sentAt", "desc")
      .limit(150)
      .get();
    if (!messagesSnap.empty) {
      await markMessagesSeenForUid(uid, messagesSnap.docs);
    }
    await markThreadUnreadZero(chatRef, uid);
    return { data: { threadId, unreadCount: 0 } };
  }

  static async acknowledgeDelivered(uid, threadId, payload = {}) {
    const { chatRef } = await ensureChatParticipant(uid, threadId);
    const rawIds = Array.isArray(payload.messageIds) ? payload.messageIds : [];
    const messageIds = Array.from(
      new Set(
        rawIds
          .map((item) => safeString(item))
          .filter((item) => item.length > 0),
      ),
    ).slice(0, 100);

    let docs = [];
    if (messageIds.length > 0) {
      const snaps = await Promise.all(
        messageIds.map((messageId) => chatRef.collection("messages").doc(messageId).get()),
      );
      docs = snaps.filter((snap) => snap.exists);
    } else {
      const recent = await chatRef
        .collection("messages")
        .orderBy("sentAt", "desc")
        .limit(80)
        .get();
      docs = recent.docs;
    }

    const deliveredCount = await markMessagesDeliveredForUid(uid, docs);
    return {
      data: {
        threadId,
        deliveredCount,
      },
    };
  }

  static async getThreadById(uid, threadId) {
    const { chatSnap, participants } = await ensureChatParticipant(uid, threadId);
    const peerUid = participants.find(u => u !== uid);
    const userActiveTimes = await fetchUserActiveTimes([peerUid]);
    const mapped = mapThreadDoc(chatSnap, uid, userActiveTimes);
    if (!mapped) {
      const error = new Error("chat not found");
      error.status = 404;
      throw error;
    }
    return { data: mapped };
  }
}

export default ChatService;
