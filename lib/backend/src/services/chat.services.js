...
function mapAndSortThreads(docs, currentUid) {
  return docs
    .map((doc) => mapThreadDoc(doc, currentUid))
    .filter(Boolean)
    .sort((a, b) => {
...
class ChatService {
  ...
  static async listThreads(uid, pagination) {
    ...
    try {
      if (canUseCursor) {
        ...
        const threads = mapAndSortThreads(docs, uid);
        return {
          data: threads,
          pagination: {
      ...
      const snap = await db
        ...
        .get();
      const threads = mapAndSortThreads(snap.docs, uid);
  ...
  static async getThreadById(uid, threadId) {
    const { chatSnap } = await ensureChatParticipant(uid, threadId);
    const mapped = mapThreadDoc(chatSnap, uid);
    if (!mapped) {
      const error = new Error("chat not found");
      error.status = 404;
      throw error;
