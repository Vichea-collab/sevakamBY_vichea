...
class OrderService {
  ...
  static async getFinderOrders(uid, pagination, filters = {}) {
    ...
    if (canUseCursor) {
      ...
      const docs = snap.docs.length > normalized.limit
        ? snap.docs.slice(0, normalized.limit)
        : snap.docs;
      const hasNextPage = snap.docs.length > normalized.limit;
      const items = docs
        .map(mapOrderDoc)
        .filter((row) =>
          statusFilters.length === 0
  ...
  static async getProviderOrders(uid, pagination, filters = {}) {
    ...
    if (canUseCursor) {
      ...
      const filteredItems = mergedItems.filter((row) =>
        statusFilters.length === 0
          ? true
          : statusFilters.includes(normalizeStatus(row.status)),
      );
      const hasNextPage = filteredItems.length > normalized.limit;
      const items = filteredItems.slice(0, normalized.limit);
      return {
        data: items,
        pagination: {
          page: normalized.page,
