...
async function enrichWithProviderMeta(items) {
  ...
  for (const chunk of chunks) {
    ...
    snaps.forEach((snap) => {
      ...
      const data = snap.data() || {};
      const photoUrl = (data.photoUrl || data.picture || "").toString().trim();
      if (photoUrl) {
        userPhotoMap[snap.id] = photoUrl;
      }
    });
  }
  ...
  return items.map((item) => {
    ...
    const avatarUrl = (userPhotoMap[item.providerUid] || item.providerAvatarUrl || "").toString();
    if (tier !== "basic") {
...
function resolveTier(value) {
  ...
  if (tier === "professional" || tier === "elite") return tier;
  return "basic";
}

function maxPhotosForTier(tier) {
...
function enforcePortfolioPhotoLimit(portfolioPhotos, tier) {
  const maxPhotos = maxPhotosForTier(tier);
  if (maxPhotos < 0) return portfolioPhotos;
  return portfolioPhotos.slice(0, maxPhotos);
}
...
class PostService {
  ...
  static async createProviderOffer(uid, user, payload) {
    ...
    const primaryService = services.length > 0 ? services[0] : "";
    const subscriptionTier = resolveTier(providerRow?.subscription?.tier);
    const portfolioPhotos = enforcePortfolioPhotoLimit(
      normalizePortfolioPhotosInput(payload),
      subscriptionTier,
    );
    ...
    const item = {
      ...
      providerAvatarUrl: userRow.photoUrl || user?.photoUrl || user?.picture || "",
  ...
  static async updateProviderOffer(uid, _user, postId, payload) {
    ...
    const providerSnap = await db.collection("providers").doc(uid).get();
    const providerRow = providerSnap.exists ? providerSnap.data() || {} : {};
    const subscriptionTier = resolveTier(providerRow?.subscription?.tier || row.subscriptionTier);
    ...
    if (payload.portfolioPhotos !== undefined) {
      updates.portfolioPhotos = enforcePortfolioPhotoLimit(
        normalizePortfolioPhotosInput(payload),
        subscriptionTier,
      );
    }
