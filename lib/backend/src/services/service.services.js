...
function normalizeServiceRecord(row = {}) {
    const ratingCount = Math.max(0, toNumber(row.ratingCount, 0));
    const ratingSum = Math.max(0, toNumber(row.ratingSum, 0));
    const fallbackRating = toNumber(row.rating, 0);
    ...
    return {
        ...row,
        ratingCount,
        ratingSum: Number(ratingSum.toFixed(1)),
        rating,
    };
}

class ServiceService {
    static invalidateActiveServicesCache() {
    ...
    static async _getActiveServices(force = false) {
        ...
        const items = servicesSnap.docs.map((doc) => normalizeServiceRecord({ id: doc.id, ...doc.data() }));
        _activeServicesCache = {
    ...
    static async getServiceById(id) {
        ...
        return { data: normalizeServiceRecord({ id: serviceSnap.id, ...serviceSnap.data() }) };
    }
