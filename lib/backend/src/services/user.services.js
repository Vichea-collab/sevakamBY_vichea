...
function resolveSupportTicketMeta(categoryValue, subcategoryValue) {
    const category = safeString(categoryValue).toLowerCase();
    const normalizedCategory = SUPPORT_TICKET_CATEGORY_OPTIONS[category] ? category : "other";
...
class UserService {
    ...
    static async createHelpTicket(uid, payload) {
        ...
        const supportMeta = resolveSupportTicketMeta(payload.category, payload.subcategory);
        ...
        const autoReplyCreatedAt = admin.firestore.Timestamp.fromMillis(Date.now() + 1);
